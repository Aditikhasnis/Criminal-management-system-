...
class Database:
    def __init__(self):
        self.conn = mysql.connector.connect(
            host='localhost',
            user='root',
            password='aditi031078',
            database='criminal_management'  # Modify the database name
        )
...
class Login:
    ...
    def login(self):
        ...
        if (username == "root" or username == "leaf" or username == "branch") and password == "123":
...
class Criminal:
    def __init__(self,root):
      ...
      # Criminal Number Entry
       ...
       #Criminal Name:
        ...
        #Birthmark
        lbl_bm= Label(Upper_frame, text='Evidence_found:', font=('arial', 11, 'bold'), background='white')
        ...
        #father NAME:
        lbl_fathername= Label(Upper_frame, text='Lawyer:', font=('arial', 11, 'bold'), background='white')
    ...
    def add_data(self):
      ...
      if self.var_case_id.get()=="":
        ...
        try:
          conn=mysql.connector.connect(host='localhost',username='root',password='aditi031078',database='criminal_management')
        ...
        #     error_code = 1  # Indication (error code) for "Age must be between 18 and 120"
        ...
        # # Query the age_error_messages table for the error message based on the error code
        #     self.my_cursor.execute('SELECT error_message FROM age_error_messages WHERE error_code=?', (error_code,))
        ...
        #   age = self.var_age.get()
        #   if age < 18 or age > 120:
        #     error_code = 2  # Indication (error code) for "Age must be between 18 and 120"
        #     error_message = self.get_error_message(error_code)
        #     ttk.messagebox.showerror("Error", error_message,PanedWindow)
        #     return
        #   if not age.isdigit():
        #     error_code = 1  # Indication (error code) for "Age must be a positive integer"
        #     error_message = self.get_error_message(error_code)
        #     ttk.messagebox.showerror("Error", error_message,PanedWindow)
        #   age = int(age)  # Convert age to an integer for further validation

        # Check if age is within the valid range
        #   if age < 18 or age > 120:
        #     error_code = 2  # Indication (error code) for "Age must be between 18 and 120"
        #     error_message = self.get_error_message(error_code)
        #     ttk.messagebox.showerror("Error", error_message)
        #     return
        ...
        # Insert the data into the database using SQL (you'll need your database connection and SQL code here)
        # def get_error_message(self, error_code):
        # # Query the error_messages table for the error message based on the error code
        #  self.my_cursor.execute("SELECT error_message FROM error_messages WHERE error_code=%s", (error_code,))
        #  result = self.my_cursor.fetchone()
        #  if result:
        #     return result[0]
        #  else:
    ...
    def delete_data(self):
      if self.var_case_id.get()=="":
        ...
        try:
          ...
          self.clear_data()
          conn.close()
          messagebox.showinfo('Success','Criminal record successfull deleted')
        except Exception as es:
          messagebox.showerror('Error',f'due to{str(es)}') 
